    ...
    try {
        ...
        const jsScenario = {
            instructions: [
                ...
                { "wait": 15000 },
                { "evaluate": "const els = document.querySelectorAll('div, p, span'); for (const el of els) { if (el.innerText && el.innerText.trim() === 'Please log in to use chart features') { let box = el; while (box && box.tagName !== 'BODY') { if (box.querySelector('button') || box.innerText.includes('Login')) { box.style.setProperty('display', 'none', 'important'); let parent = box.parentElement; if (parent && parent.tagName !== 'BODY' && parent.id !== 'root' && parent.innerText && parent.innerText.length < 500) { parent.style.setProperty('display', 'none', 'important'); } break; } box = box.parentElement; } break; } }" },
                { "evaluate": "const canvases = document.querySelectorAll('canvas'); let hasVisible = false; for (const c of canvases) { if (c.offsetHeight > 50) hasVisible = true; } if(!hasVisible || !document.body.innerText.includes('Liquidation') || document.body.innerText.includes('Just a moment') || document.body.innerText.includes('Cloudflare')) throw new Error('CHART_NOT_FOUND');" }
            ]
        };
