...
async function takeCoinankScreenshot(ticker) {
    ...
    try {
        ...
        const jsScenario = {
            instructions: [
                { "evaluate": "const style = document.createElement('style'); style.innerHTML = '* { filter: none !important; backdrop-filter: none !important; }'; document.head.appendChild(style);" },
                { "wait": 15000 },
                { "evaluate": "const els = document.querySelectorAll('div'); for (const el of els) { if (el.innerText && el.innerText.includes('Please log in to use chart features')) { let popupContainer = el; while (popupContainer.parentElement && popupContainer.parentElement.innerText && popupContainer.parentElement.innerText.includes('Please log in to use chart features') && popupContainer.parentElement.innerText.length < 500 && popupContainer.parentElement.id !== 'root' && popupContainer.parentElement.tagName !== 'BODY') { popupContainer = popupContainer.parentElement; } popupContainer.style.setProperty('display', 'none', 'important'); break; } }" },
                { "evaluate": "const canvases = document.querySelectorAll('canvas'); let hasVisible = false; for (const c of canvases) { if (c.offsetHeight > 50) hasVisible = true; } if(!hasVisible || !document.body.innerText.includes('Liquidation') || document.body.innerText.includes('Just a moment') || document.body.innerText.includes('Cloudflare')) throw new Error('CHART_NOT_FOUND');" }
            ]
        };
